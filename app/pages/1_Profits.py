...
customer_acquirement_cost = st.number_input('Customer Acquisition Cost', value=200)
...
retention_success_rate = retention_stats[retention_stats > 0].count() / len(retention_stats)

churned = data[data['Churn'] == 'Yes']

normal_cost = customer_acquirement_cost * churned['Churn'].count()
our_cost = ((retention_success_rate * churned['Churn'].count()) * customer_acquirement_cost +
            ((1-retention_success_rate) * churned['Churn'].count() * cost_of_retention))
